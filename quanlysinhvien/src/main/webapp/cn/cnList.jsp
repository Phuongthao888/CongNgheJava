...
<body>
	<br>
	<center>
		<h3>Quản lý danh sách chuyên ngành</h3>
	</center>
	...
	<div class="card-add">
		...
		<a href="${pageContext.request.contextPath}/newCNServlet">
			&nbsp&nbsp Thêm chuyên ngành</a>
	</div>
	<br>
					...
					<div class="col bottom">
						<div class="card" style="width: 400px;">
							<div class="card-body card-color">
								<h5 class="card-title">Mã chuyên ngành: ${cn.macn}</h5>
								<h6 class="card-subtitle mb-2 text-muted">Tên chuyên ngành:
									${cn.tencn}</h6>
								<h6 class="card-subtitle mb-2 text-muted">Trực thuộc khoa:
									${cn.khoaFK.makhoa} - ${cn.khoaFK.tenkhoa}</h6>
								<hr>
								<a href="editCNServlet?code=${cn.macn}" class="btn btn-info">Cập
									nhật</a> <a
									href="./cn/deleteCN.jsp?code=${cn.macn}&tenchuyennganh=${cn.tencn}"
			...
			<tbody>
				<c:forEach items="${cnList}" var="cn">
					<tr style="text-align: center;">
						<td>${cn.macn}</td>
						<td>${cn.tencn}</td>
						<td>${cn.khoaFK.makhoa}-${cn.khoaFK.tenkhoa}</td>
						<td><a href="editCNServlet?code=${cn.macn}"
		...
		document
				...
				.addEventListener(
						...
						function() {
							...
							tableContent.style.display = (tableContent.style.display === 'none' || tableContent.style.display === '') ? 'block'
									: 'none';
						});
	</script>
