...
<body>
	...
	<div class="card-add">
		...
		<a href="${pageContext.request.contextPath}/newKhoaServlet">
	...
	<%
	    List khoaList = (List) request.getAttribute("khoaList");
	    int count = 0;
	    if(khoaList != null){
	        count = khoaList.size();
	    }
	%>
		<div class="count">
		 <b>Có <%= count %> khoa đào tạo trong danh sách</b>	
		</div>
					...
					<div class="col bottom">
						<div class="card" style="width: 400px;">
							<div class="card-body card-color">
								...
								<h6 class="card-subtitle mb-2 text-muted">Tên khoa:
									${khoa.tenkhoa}</h6>
								<hr>
								<a
									href="editKhoaServlet?code=${khoa.makhoa}&tenkhoa=${khoa.tenkhoa}"
									class="btn btn-info">Cập nhật</a> <a
									href="./khoa/deleteKhoa.jsp?code=${khoa.makhoa}&tenkhoa=${khoa.tenkhoa}"
...
<script>
    ...
    document.getElementById('iconListToggle').addEventListener('click', function() {
        // Toggle the display property of the content container
        var cardListContent = document.getElementById('cardListContent');
        cardListContent.style.display = (cardListContent.style.display === 'none' || cardListContent.style.display === '') ? 'block' : 'none';

        // Enable the table list toggle button
        var tableListToggle = document.getElementById('tableListToggle');
        tableListToggle.disabled = false;
    });
</script>
...
<script>
    ...
    document.getElementById('tableListToggle').addEventListener('click', function() {
        var tableContent = document.getElementById('tableContent');

        // Đảo ngược trạng thái và hiển thị/ẩn dữ liệu tùy thuộc vào trạng thái mới
        isTableVisible = !isTableVisible;
        tableContent.style.display = isTableVisible ? 'block' : 'none';

        // Ẩn script dạng card nếu hiển thị script dạng table và vô hiệu hóa nút toggle của nó
        if (isTableVisible) {
            var cardListContent = document.getElementById('cardListContent');
            cardListContent.style.display = 'none';

            var iconListToggle = document.getElementById('iconListToggle');
            iconListToggle.disabled = true;
        }
    });

    // Thêm sự kiện 'click' cho script dạng card để chuyển đổi sang script dạng table
    document.getElementById('cardListToggle').addEventListener('click', function() {
        var cardListContent = document.getElementById('cardListContent');
        cardListContent.style.display = 'none';
        ...
        // Enable the table list toggle button và vô hiệu hóa nút toggle của script dạng card
        var tableListToggle = document.getElementById('tableListToggle');
        tableListToggle.disabled = false;
